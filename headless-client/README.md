...
[servers/stitching/.env](servers/stitching/.env)
```
COREMEDIA_ENDPOINT=https://headless-server-preview.cm95.coremedia.training/graphql
```
If on GitHub Codespaces, also need to change the `VITE_API_ENDPOINT` of the Spark app.
This environment variable must point to the GraphQL endpoint of the stitching server.

[apps/spark/.env](apps/spark/.env)
```
VITE_API_ENDPOINT=https://<your-codespace-hostname-prefix>-4000.app.github.dev/graphql
```

Replace <your-codespace-hostname-prefix> by the unique codespace hostname prefix. This prefix
consists of two random words and a hash (example: "zany-chainsaw-699x9qqv6ph4wwv"), and is part 
of your codespace URL.
...
You can now start the workspace including the spark app:
```
pnpm start
```
