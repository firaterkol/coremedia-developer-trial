...
const { CoreMediaChunkMappingPlugin } = require("../plugins/CoreMediaChunkMappingPlugin");
...
const flattenedThemeDependencies = getFlattenedDependencies(themeConfig.pkgPath, getIsSmartImportModuleFor(null));
...
function buildEntryPoint(variant, paths) {
  ...
  const autoActivateDependencies = flattenedThemeDependencies.filter(getIsSmartImportModuleFor(variant));
  const autoActivateEntryPoints = autoActivateDependencies.map(getInitJs).filter((dependency) => !!dependency);
...
If you want to reuse this theme in your project, please create a copy of the theme folder and change the package name in the "package.json" so it does no longer start with "${EXAMPLE_SCOPE}/". It is also adviced to change the theme name.`,
  ...
  const dependenciesToExampleThemes = flattenedThemeDependencies.filter(isExampleThemeModule);
  if (dependenciesToExampleThemes.length > 0) {
    log.warn(
      `You have dependencies to the following examples themes:
${dependenciesToExampleThemes.map((nodeModule) => `- ${nodeModule.getName()}`).join("\n")}
If you want to use these themes in your project, please create a copy and change their package name so it does no longer start with "${EXAMPLE_SCOPE}/". It is also adviced to change the name of the theme in the theme configuration.`,
    ...
    const dependenciesToExampleBricks = flattenedThemeDependencies.filter(isExampleBrickModule);
    if (dependenciesToExampleBricks.length > 0) {
      log.warn(
        `You have dependencies to the following examples bricks:
${dependenciesToExampleBricks.map((nodeModule) => `- ${nodeModule.getName()}`).join("\n")}
...
const babelConfigFile = !fs.existsSync(path.resolve(themeConfig.path, "babel.config.js"))
